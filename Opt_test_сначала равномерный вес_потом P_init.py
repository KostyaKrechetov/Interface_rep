...
def save_cumul(*args):
    ...
    plt.savefig(
        os.path.join(result_directory, str(args[1]) + str(args[0]) + 'накопленная добыча' + '.png'),
        dpi=300)
    plt.close(fig)
...
def save_hist(*args):
    ...
    plt.savefig(os.path.join(result_directory, str(args[1]) + str(args[0]) + 'histogram' + '.png'),
                dpi=300)
    plt.close(fig)
...
def save_plot(*args, prediction=False):
    ...
    fig.autofmt_xdate()
    # plt.show()
    plt.savefig(os.path.join(result_directory, str(args[1]) + '  ' + str(args[0]) + '.png'), dpi=300)
    plt.close(fig)
...
def loss_func_maximize_P_init_only(p_init_, *args):
    ...
    # input_list = [((delta_dates + comparison_delta_dates)[i] / 24, p_init_[0] - bh_pressures[i]) for i in
    #               range(len(delta_dates + comparison_delta_dates))]
    num_calls += 1
    Q = np.array([])
    for t in time_points:
...
def optimize_common(alpha):
    global time_points, input_list, Q, weight, x_, best, num_calls, losses, Q_arr, Q_arr_comp

    Q_weighed = []
    for i in range(len(Q_arr)):
        Q_weighed.append(alpha[i] * Q_arr[i])
    Q_result = np.sum(Q_weighed, axis=0)
    dev = (Q_result - Q_liq[start_Q_liq:end_Q_liq + 1]) * (Q_result - Q_liq[start_Q_liq:end_Q_liq + 1])
    loss = np.sqrt(np.sum((dev * weight)) / considered_length)
    # print('loss: ' + str(loss) + '  num: ' + str(num_calls))
    print('loss: ' + str(loss) + '\t alpha: ' + str(alpha))
    return loss


# notes_f = open('Проверка на реальных данных\\30 скважин от Дениса\\notes.txt', 'r')
# notes = []
# lines_ = notes_f.readlines()
# for line in lines_:
#     line = str(line).replace(' ', '')
#     words = str(line).split('-')
#     no_underl = words[0].partition('_')
#     words[0] = str(no_underl[0])  # well_num
#     words[2] = ''.join(i for i in words[2] if not i.isdigit())
#     words[2] = str(words[2]).replace('.', '')
#     notes.append(words)

# xl_tech_regime = pd.ExcelFile(
#     'Проверка на реальных данных\\30 скважин от Дениса\\Техрежим ОАО  Газпромнефть-Ноябрьскнефтегаз  апрель 2020 (005).xlsx')
xl_tech_regime = pd.ExcelFile(
...
#     'Проверка на реальных данных\\30 скважин от Дениса\\Печать справочника физико-химических свойств пласта за декабрь 2019 г по предприятию АО Газпромнефть-ННГ.xlsx')
xl_pechat = pd.ExcelFile(
...
# xl_P_bh = pd.ExcelFile('Проверка на реальных данных\Простой случай\\Тест входных данных - P_bh.xlsx')
# xl_P_bh = pd.ExcelFile('Проверка на реальных данных\От Элины\\2007 _ готовое.xlsx')
# xl_P_bh = pd.ExcelFile('Проверка на реальных данных\Шахматка от Дениса\\163.xlsx')
# xl_P_bh = pd.ExcelFile('Проверка на реальных данных\\1200 Холмогорское\\P_bh.xlsx')
# xl_P_bh = pd.ExcelFile('Проверка на реальных данных\\10 скважин от Дениса\\10_wells.xlsx')
# xl_P_bh = pd.ExcelFile('Проверка на реальных данных\\30 скважин от Дениса\\day.xlsx')
#     xl_P_bh = pd.ExcelFile('Проверка на реальных данных\\Отдельное (месторожд)\\otdelnoe\\day.xlsx')
...
comparison_start = datetime(2020, 2, 1)
P_bh_simplification_coeff = 12
Q_liq_simplification_coeff = 9
considered_period = 365  # Количество дней, в рамках которых происходит адаптация
comparison_period = 90  # Количество дней в конце истории, которые остаются нетронутыми для проверки прогноза
coef = 2
fill_in_gaps = True # Заполнить пропуски дебита жидкости при прогнозе
delete_common_dates = True
equal_time = False  # Заменить всю предысторию работы скважины на эквив. время и забойку
...
for index_, elem in enumerate(res_prms[srez_l:srez_r]):
    ...
    #     f'RESULT\\Отдельное (месторождение)\\забойное по Нд\\Сначала равном. вес потом нач.пл.давл.\\RESULT ' + str(
    #         datetime.now().strftime('%Y-%m-%d-%H.%M.%S')))
    result_directory = os.path.join(
        f'RESULT\\Отдельное (месторождение)\\забойное по Нд\\Сначала равном. вес потом нач.пл.давл.\\' + str(
            datetime.now().strftime('%Y-%m-%d-%H.%M.%S')) + ' well ' + str(well_num))
    ...
    for i in reversed(range(len(datetimes_P_bh))):
        if datetimes_P_bh[-1] - datetimes_P_bh[i] < timedelta(days=comparison_period):
            # if datetimes_P_bh[i] > date(2020, 2, 1):
            end_P_bh = i
            continue
    ...
    for i in reversed(range(len(datetimes_P_bh))):
        if datetimes_P_bh[-1] - datetimes_P_bh[i] < timedelta(days=comparison_period):
            # if datetimes_P_bh[i] > datetime.date(2020, 2, 1):
            end_P_bh = i
            try:
    ...
    # bounds = [(0.1, 10), (0, 5), (0.5, 5), (300, 2000), (300, 2000), (max(bh_pressures), max(bh_pressures)*3)]
    if WM[res_prm[1]] == 2:
    ...
    # x_0 = 0.67, 0, 1, 1700, 2300, 203.6
    # x_2 = [(elem[1] - elem[0])/1.1 + elem[0] for elem in bounds]
    # loss_func(x_2)
    # save_plot('-Diff.Ev.-prediction-', q_)

    # x_2 = 1.37580374e+01, 3.61887076e+00, 5.41192147e-01, 1.41360129e+04, 4.66977836e+02, 8.40530191e+01, 1.42191413e+01, 6.30489449e+03
    ...
    for t in [1]:
        ...
        options = {'maxiter': 60, 'final_sten_size': 0.0000001}
        for i in range(1):
            if WM[res_prm[1]] == 2:
            ...
            # x_init = [np.random.uniform(bounds[i][0], bounds[i][1]) for i in range(len(bounds))]
            res = gpse.gpse(fun=loss_func_maximize, x0=x_init, bounds=bounds, options=options, params=params)
            x = res[0]
            loss_func(x)
            save_plot('-GPSE-fixed_p_init', 1)
            ...
            # results_P_initial.append(res[0][4])
            # plt.plot(res[3])
            # plt.show()
            ...
            for t in time_points:
                ...
                #               w=x[3], l=x[4], wf=0.5, lf=0.5)
                # q = Pwf_Ql_Qt(t=t, Regimes=input_list, k=x[0], regimes_flag=1,
                #               skin=x[1], h=2.8, ct=9.66799e-5, mu=0.369, bl=1.01,
                #               rw=0.1, phi=0.158, Wellmodel=WM,
                #               ReservoirShape=0, BoundaryType=0, xf=x[6],
                #               kv_kh=x[2], Fc=x[7],
                #               w=x[3], l=x[4], wf=0.5, lf=0.5)
                # q = Pwf_Ql_Qt(t=t, Regimes=input_list, k=x[0], regimes_flag=1,
                #               skin=x[1], h=2.8, ct=9.66799e-5, mu=0.369, bl=1.01,
            ...
            save_plot('-GPSE-prediction-', q_, prediction=True)
            save_hist('-GPSE-prediction-', q_)
            save_cumul('-GPSE-prediction-', q_)
            dev = (Q - Q_liq[start_Q_liq:end_Q_liq + 1]) * (Q - Q_liq[start_Q_liq:end_Q_liq + 1])
            erro = np.sqrt(np.sum((dev * weight)) / considered_length)
            # if q_ == 1.7 and len(Q_comparison) > 0:
    ...
    # params = {'m': 3, 'step_size_coeff': 10}
    # options = {'maxiter': 50, 'final_sten_size': 0.00001}
    # res_common = gpse.gpse(fun=optimize_common, x0=[0.5, 0.5], bounds=[(0, 1), (0, 1)], options=options, params=params)
    # print(res_common[0][0], res_common[0][1])
    # Q_result = res_common[0][0] * Q_first + res_common[0][1] * Q_second
    # Q_result_comp = res_common[0][0] * Q_first_comp + res_common[0][1] * Q_second_comp
    # Q = np.copy(Q_result)
    # Q_comparison = np.copy(Q_result_comp)
    # save_plot('-Common-prediction-', 1.7, prediction=True)
    # save_hist('-Common-prediction-', 1.7)
    # save_cumul('-Common-prediction-', 1.7)
    if len(Q_comparison) > 0:
        write_excel(index_ + 2)
    # ---------------------------output-----------------------------------------------------
    if True:
        f = open(os.path.join(result_directory, str(well_num) + ' - parameters_log.txt'), 'w')
        for i in range(len(results_fun)):
            ...
            # f.write('\tAdapted P_initial = ' + str(results_P_initial[i]) + '\n')
            ...
            #     print('losses : \n' + str(losses[i]) + '\n')
            f.write('\n\n')
        # f.write('coefficients ensemble: ' + str(res_common[0][0]) + '  ' + str(res_common[0][1]))
